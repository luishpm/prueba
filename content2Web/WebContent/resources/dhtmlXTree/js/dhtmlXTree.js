...
/***** Added by Art Santos *****/
 ...
 dhtmlXTreeObject.prototype._getAllItemsCero =function(node)
{
 var z="";
 for(var i=0;i<node.childsCount;i++)
{
 if((node.childNodes[i].unParsed)||(node.childNodes[i].childsCount>0))
{
 ...
 dhtmlXTreeObject.prototype.getAllItemsCero =function(){
 return this._getAllItemsCero(this.htmlNode);
}


 
 dhtmlXTreeObject.prototype._getChildlessFirstLevel =function(node)
{
 var z="";
 for(var i=0;i<node.childsCount;i++)
{
 if((node.childNodes[i].unParsed)||(node.childNodes[i].childsCount>0))
{
 ...
 dhtmlXTreeObject.prototype.getChildlessFirstLevel =function(){
 return this._getChildlessFirstLevel(this.htmlNode);
}



dhtmlXTreeObject.prototype._getAllCheckedFirstLevel=function(node,list,mode){
 ...
 for(var i=0;i<node.childsCount;i++)
{
 ...
 if(node.childNodes[i].checkstate==mode)
 if(!node.childNodes[i].nocheckbox){
  if(!z)z=node.childNodes[i].id;
  else z+=this.dlmtr+node.childNodes[i].id;
 }
 ...
 dhtmlXTreeObject.prototype.getAllCheckedFirstLevel=function(){
 var temp= this._getAllCheckedFirstLevel(this.htmlNode,"",1);
 if(temp!="")temp+=this.dlmtr;
 return temp+this._getAllCheckedFirstLevel(this.htmlNode,"",2);
 
 //return this._getAllCheckedFirstLevel(this.htmlNode,"",1);
}
...
dhtmlXTreeObject.prototype._deleteAllItems=function(node){
var ids = this._getAllItemsCero(node);
if (ids!=''){
	var nodes = ids.split(',');
	for(var i=0;i<nodes.length;i++)
	{
		this._deleteItem(nodes[i])
	}
}
};

 dhtmlXTreeObject.prototype.deleteAllItems=function(){
 return this._deleteAllItems(this.htmlNode);
}
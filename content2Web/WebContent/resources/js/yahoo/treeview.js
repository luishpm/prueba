 ...
 * @constructor
 */
YAHOO.widget.Node = function(oData, oParent, expanded, idFlujo) {
    if (oData) { this.init(oData, oParent, expanded, idFlujo); }
};
...
YAHOO.widget.Node.prototype = {
    ...
    /**
     * Se agrego el tipo 
	 * Tipo 0 folder, tipo 1 archivo...
     */
	stringType: ["DIR","IMAGE","FILE"],
    nodeType: 0,
    size: null,
	tieneHijos: true,
	id: null,
...
/**
    ...
    */
     ...
     * @param expanded {boolean} the initial expanded/collapsed state
     */
    init: function(oData, oParent, expanded, idFlujo) {
        this.data       = oData;
        this.children   = [];
    ...
    */
     ...
     */
    getElId: function() {
        return this.stringType[this.nodeType] + this.index;
    },
    ...
    getChildrenElId: function() {
        return this.stringType[this.nodeType] + "Child" + this.index;
    },
    ...
    getToggleElId: function() {
        return this.stringType[this.nodeType] + "Toogle" + this.index;
    },
    ...
    */
    ...
    getStyle: function() {		
        if (this.isLoading) {
            return this.stringType[this.nodeType] + "loading";
        } else {
			// location top or bottom, middle nodes also get the top style
			var loc = (this.nextSibling) ? "top" : "bottom";			
			if (this.nodeType==0){
				...
				if (this.tieneHijos)
					...
					// if (this.hasChildren(true)) {
						type = (this.expanded) ? "Collapsable" : "Expandible";
					} else type = "NoChildren"
				
				//return "ygtv" + loc + type;			
	            return this.stringType[this.nodeType] + loc + type;
			}else
				return this.stringType[this.nodeType] + loc;
        }
    },
    ...
    getHoverStyle: function() { 
        var s = this.getStyle();		
        if (this.hasChildren(true) && !this.isLoading && this.nodeType==0 && this.tieneHijos) { 
            s += "Hover"; 
        }
        return s;
    },
 ...
 * @param expanded {boolean} the initial expanded/collapsed state
 */
YAHOO.widget.TextNode = function(oData, oParent, expanded, idFlujo) {
    // this.type = "TextNode";
    if (oData) { 
        var labelData = oData;
        if (idFlujo){
            if (idFlujo == "?" && oParent.idFlujo != "?") idFlujo = oParent.idFlujo;
            if (idFlujo != "?")
                labelData = "<b>"+labelData+"</b>";
        }
        this.init(oData, oParent, expanded, idFlujo);
        this.setUpLabel(labelData);
    }
...
YAHOO.widget.TextNode.prototype.getNodeHtml = function() { 
    ...
    for (i=0;i<this.depth;++i) {
        ...
        sb[sb.length] = '<td class="' + this.getDepthStyle(i) + '" nowrap>&#160;</td>';
    }
    ...
    sb[sb.length] = ' onclick="javascript:' + this.getToggleLink() + '" nowrap>';
    ...
    */
    ...
    sb[sb.length] = ' oncontextmenu="' + this.onContextMenu + '"';
    if (this.hasChildren(true)) {
